...
详见归档需求文档 `需求归档/全功能开发阶段一与阶段二验收整改需求.md` §18.4：
...
按归档文档《全功能开发阶段一与阶段二验收整改需求》完成3大阻塞问题修复和前端全量接入。
...
实现《素材库后端开发与前端对接需求书》定义的全部后端能力，与已完成的前端素材库页面对接，形成全局素材的基础管理闭环：创建、列表查询、搜索筛选、详情、局部更新、删除、标签管理与预览图上传/读取/替换/删除。

### 开发分支

`dev-260728-GLM5.2-materials-backend`（从 `GLM-MAIN` 拉取新建）

### 实际解决方案

1. **数据库层**：新增 `materials` / `material_tags` / `material_tag_links` 三表，沿用 `DatabaseManager` 当前激活 SQLite 数据库，不新建独立库。`materials` 含同类型内名称唯一约束、类型与状态 CHECK 约束、三个索引；关联表双向 `ON DELETE CASCADE`。保持 WAL 模式不改。
2. **数据访问层**：在 `DatabaseManager` 新增 7 个方法。`list_materials` 在 SQL 层完成搜索（name/description/content/tag 四字段 LIKE，转义 `%`/`_`/`\`）、筛选、排序、分页，一次 JOIN 聚合返回标签避免 N+1。`update_material` 区分字段未提交与明确清空。`delete_material` 级联清理标签关联与图片目录，路径钳制防止越权删除。
3. **接口层**：新增 9 个路由覆盖素材 CRUD、标签候选、预览图上传/读取/删除。Pydantic 模型带字段级校验，错误响应返回用户可读中文，不暴露 SQLite 错误或绝对路径。
4. **图片处理**：Pillow 按文件内容解码校验（不信任扩展名/MIME），20MB 上限，最长边 16384 像素上限，生成 512px WebP 缩略图（质量 82），先写临时文件再原子替换，上传失败保留旧图。
5. **依赖**：`requirements.txt` 新增 `Pillow==12.3.0`、`python-multipart==0.0.32`。
...
本次以现有全局人物、形象变体、全局规格和规格值数据结构为准，只补齐人物库当前使用流程。

### 实施

- 人物详情默认读取默认变体的全部规格值。
- 切换形象变体时读取对应规格值，不刷新整个人物弹窗。
- 每项规格可编辑提示词、LoRA 文件、LoRA 权重、模型覆盖和备注。
- 保存过程显示状态，失败保留输入并显示错误。
- 支持将 LoRA 权重明确清空为 `NULL`。
- 修复隐藏的新建变体/规格表单仍被 CSS 显示的问题。
- 修复人物详情顶部规格统计读取错误字段的问题。

### 验证

- 后端与接口自动化测试 107 项全部通过。
- `app.js` 与 `runtime-api.js` 语法检查通过。
- 8111 浏览器验收通过：保存规格内容后关闭并重新打开人物，全部字段正确读回。
- 浏览器控制台无错误或警告。
- 验收产生的人物、规格和项目已从测试数据库清理。

## 角色查询首次进入性能优化（GPT-MAIN）

更新日期：2026-07-28

### 问题与处理

- 首次索引原实现会一次性读取 244,932 行 CSV，Python 峰值内存约 144 MB。
- 页面会一次性读取并创建 22,272 个作品系列选项，导致浏览器主线程明显卡顿。
- 改为流式批量导入，并在数据写入完成后统一创建普通索引与 FTS5 索引。
- SQLite 查询缓存改存于本机 `LOCALAPPDATA/Atelier/character-database`，避免在共享盘随机读写。
- 自动发现项目数据目录、环境变量或本机“提示词整理”目录中的角色 CSV。
- 作品系列改为最多 50 条的按需前缀查询，前端使用轻量自动补全。
- 读取连接显式关闭；测试环境创建应用时不再启动真实角色库后台导入。

### 验证

- 真实 39 MB CSV 冷构建：8.1 秒降至 4.25 秒。
- Python 峰值内存：144 MB 降至 1.3 MB。
- 作品系列候选查询约 0.5 ms。
- 后端 110 项测试全部通过。
- 浏览器首屏仅 524 个 DOM 节点、50 行结果、0 个预加载系列选项。
- 角色搜索、系列自动补全、系列筛选和懒加载通过；浏览器控制台无错误。
...
- 移除素材库和素材详情页的展示假数据，改为真实接口驱动。
- 完成素材列表、空状态、后端未完成状态、搜索、类型/状态/标签筛选、排序和分页加载。
- 完成素材新建、详情编辑、局部保存、删除、标签编辑和未保存修改保护。
- 完成预览图选择、上传、替换、移除，以及列表缩略图懒加载和加载失败降级。
- 保持素材为全局资源，本期不联动项目、剧本画布、人物库或 ComfyUI。

### 后端边界

- 本次没有实现素材库后端。
- 后端编程 AI 按 `素材库后端开发与前端对接需求书.md` 实现 SQLite、FastAPI、图片存储和自动化测试。

### 验证

- `app.js` 与 `runtime-api.js` JavaScript 语法检查通过。
- 使用临时内存模拟接口完成列表筛选、新建、详情修改、标签保存、删除和未保存修改确认的浏览器验收。
- 验收页面控制台无 JavaScript 错误。
...
- 恢复最初设计的三栏结构：左侧积木、中间横向主线画布、右侧节点检查器。
- 将原始预览中的演示节点替换为真实章节和大场景数据，不显示未开发的虚假分支。
- 空项目与已有项目共用同一套画布外壳，空项目只在主线中央显示创建入口。
- 支持拖动空白区域平移、滚轮平移、`Ctrl/Command + 滚轮`缩放。
- 保留最初设计的主线工具栏，并接入缩小、恢复 100%、放大和自动整理。
- 按项目记忆画布位置与缩放比例，仅保存在浏览器，不修改业务数据。
- 节点点击后在右侧显示真实名称、所属章节、类型和可用操作。
- 保留右键改名/删除、新建以及大场景排序和跨章节移动。
- 节点继续自动对齐和吸附，不允许任意自由摆放。

### 验证

- `app.js` 与 `runtime-api.js` JavaScript 语法检查通过。
- 浏览器验收通过：真实项目与空项目三栏结构一致，缩放、节点选择、检查器联动和右键菜单正常。
- 浏览器控制台无错误或警告。
- 本次没有修改后端接口或数据库结构。
