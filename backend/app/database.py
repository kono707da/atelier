...
class DatabaseManager:
        ...
        with self._lock, self.connection(environment) as connection:
            ...
            connection.executescript(
                ...
                CREATE TABLE IF NOT EXISTS materials (
                    ...
                    CHECK (
                        validation_status IN ('verified', 'unverified')
                    )
                );

                CREATE INDEX IF NOT EXISTS idx_materials_sort
                    ON materials(sort_order);
                ...
                CREATE INDEX IF NOT EXISTS idx_material_tags_material
                    ON material_tags(material_id);

                CREATE INDEX IF NOT EXISTS idx_material_tags_tag
                    ON material_tags(tag);
    ...
    def list_materials(
        ...
        *,
        ...
        with self.connection(target_environment) as connection:
            ...
            if tag:
                clauses.append("EXISTS (SELECT 1 FROM material_tags mt WHERE mt.material_id = m.id AND mt.tag = ?)")
                params.append(tag)
            if q:
            ...
            order_map = {
                ...
                "sort_asc": "m.sort_order ASC",
            }
            order = order_map.get(sort, "m.updated_at DESC")
            rows = connection.execute(
                f"""
                SELECT m.id, m.name, m.material_type, m.description,
                       m.validation_status, m.preview_path, m.sort_order,
    ...
    def get_material(
        ...
        with self.connection(target_environment) as connection:
            row = connection.execute(
                """
                SELECT id, name, material_type, description,
                       validation_status, preview_path, sort_order,
                       created_at, updated_at
            ...
            tag_rows = connection.execute(
                "SELECT tag FROM material_tags WHERE material_id = ? ORDER BY created_at",
                (material_id,),
            ).fetchall()
            material["tags"] = [r["tag"] for r in tag_rows]
        return material
    ...
    def create_material(
        ...
        *,
        ...
        if validation_status not in ("verified", "unverified"):
            raise ValueError("验证状态无效，允许值: verified, unverified")
        target_environment = environment or self._active_environment
        now = datetime.now(timezone.utc).isoformat()
        ...
        with self._lock, self.connection(target_environment) as connection:
            max_order = connection.execute(
                "SELECT COALESCE(MAX(sort_order), 0) FROM materials"
            ).fetchone()[0]
            connection.execute(
                """
                INSERT INTO materials (id, name, material_type, description,
                    validation_status, preview_path, sort_order, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (material_id, name, material_type, description,
                 validation_status, preview_path, max_order + 1, now, now),
            )
            for tag_text in tags:
                ...
                if tag_text:
                    connection.execute(
                        """
                        INSERT INTO material_tags (id, material_id, tag, created_at)
                        VALUES (?, ?, ?, ?)
                        """,
                        (str(uuid4()), material_id, tag_text, now),
                    )
        result = self.get_material(material_id, environment=target_environment)
    ...
    def update_material(
        ...
        *,
        ...
        if all(v is None for v in (name, material_type, description, validation_status, preview_path, tags)):
        ...
        with self._lock, self.connection(target_environment) as connection:
            ...
            if preview_path is not None:
                sets.append("preview_path = ?")
                params.append(preview_path)
            sets.append("updated_at = ?")
            params.append(now)
            params.append(material_id)
            connection.execute(
                f"UPDATE materials SET {', '.join(sets)} WHERE id = ?", params
            )
            if tags is not None:
                connection.execute(
                    "DELETE FROM material_tags WHERE material_id = ?", (material_id,)
                )
                for tag_text in tags:
                    tag_text = tag_text.strip()
                    if tag_text:
    ...
    def delete_material(
        ...
        with self._lock, self.connection(target_environment) as connection:
            ...
            remaining = connection.execute(
                "SELECT id FROM materials ORDER BY sort_order ASC"
            ).fetchall()
        ...
        return {"id": material_id, "name": existing["name"]}
