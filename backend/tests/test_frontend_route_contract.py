...
class FrontendRouteContractTests(unittest.TestCase):
    ...
    def test_character_library_defaults_to_simple_editor(self) -> None:
        ...
        self.assertNotIn("character-simple-spec-create", self.source)
        self.assertNotIn('data-inline-action="create-spec"', self.source)
        self.assertIn("createCharacterSpec", self.source)
        self.assertIn('data-api-action="delete-character-spec"', self.source)
        self.assertIn('data-api-action="edit-character-spec"', self.source)
        self.assertIn("character-spec-readonly", self.source)
        self.assertIn("data-spec-prompt-output", self.source)
        self.assertIn("setCharacterSpecEditorMode(form, false)", self.source)
        self.assertIn("character-spec-mini-list", self.source)
        self.assertIn('data-api-action="select-character-spec"', self.source)
        self.assertIn("characterSpecWorkspace", self.source)
        self.assertIn("renderSelectedCharacterSpec", self.source)
        self.assertIn("个公共规格", self.source)
        self.assertIn("提示词 ${completeness}", self.source)
